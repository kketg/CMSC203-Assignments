...
/**
 * This panel is the basic pane, inside which other panes are placed. Updated
 * with JavaFX alerts and TextInputDialog by prof Kartchner
 * 
 * @author ralexander
 */
public class MainPaneFX {
	...
	MainPaneFX(double CANVAS_WIDTH, double CANVAS_HEIGHT) {
		...
		/*
		 * When the button pushed was the graph function button, user is prompted to
		 * select one of the functions, and is asked for the left and right limits
		 * (extents) to plot the function.
		 */
		...
		do {
			try {
				...
				if (returnVal < 1 || returnVal > 5) {
					...
					// between 1 and 5");
					new Alert(Alert.AlertType.ERROR, "Choice of function must be an integer between 1 and 5")
							.showAndWait();
				} else
					error = false;
